...
static uint32_t trim_table_init(struct stm32_calib *data)
{
	unsigned int min = data->cal_ref + data->cfg->trim_min;
	unsigned int max = data->cal_ref + data->cfg->trim_max;
	unsigned long last_freq = 0UL;
	unsigned long min_freq = 0UL;
	long best_diff = INT32_MAX;
	uint32_t best_trim = 0;
	uint32_t bmin = max;
	uint32_t bmax = 0;
	uint32_t trim = 0;
	int boundary = 0;

	for (trim = max; trim >= min; trim--) {
		...
		if (freq > last_freq) {
			if (bmax == 0) {
				...
				save_trim(data, boundary++, bmax, bmin);
				min_freq  = last_freq;

				if (boundary == MAX_CALIBRATION_BOUNDARY)
					panic("MAX BOUNDARY !");
			}
			last_freq = freq;
		}
		...
		if (freq < data->ref_freq)
			diff = data->ref_freq - freq;
		else
			diff = freq - data->ref_freq;
	...
	DMSG("Boundary of %s:", data->cfg->name);

	for (boundary = 0; boundary < (int)data->boundary_max; boundary++) {
		DMSG("boundary[%d].max = %d, boundary[%d].min = %d",
		     boundary, data->boundary[boundary].max,
		     boundary, data->boundary[boundary].min);
	}
...
static TEE_Result stm32mp2_clk_init_hsi_cal(struct stm32_calib_pdata *pdata)
{
	...
	best_trim = trim_table_init(data);

	data->ops->set_trim(best_trim, data->cal_ref);
